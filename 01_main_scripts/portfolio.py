...
class portfolio:
    ...
    def init_portfolio(self, init_date:date = date.today()\
        ...
        if self.eval_bench and self.benchmark_file == None:
        ...
        elif self.eval_bench and benchmark_file != None:
            self._benchmark_data = pd.DataFrame()
            try:
                self._benchmark_data = pd.read_csv(benchmark_file,encoding='utf-8',dtype='str',index_col=0)
                for i in range(len(self._benchmark_data)):
                    self._benchmark_data.iloc[i]['날짜'] = str_to_date(self._benchmark_data.iloc[i]['날짜'])
                self._benchmark_data[['종가','시가','거래량']]=self._benchmark_data[['종가','시가','거래량']].apply(pd.to_numeric)
                self._benchmark_init_val = self._benchmark_data.iloc[0]
                _date_it = self.init_date
                while True:
                    try:
                        self._benchmark_init_val = self._benchmark_data.loc[self._benchmark_data['날짜']==_date_it].iloc[0]['종가']
                        break
                    except:
                        _date_it = _date_it + td(days=1)
                if date_it != self.date:
                    verbose(self._verbose, portfolio.k_warning,
                ...
                verbose(self._verbose,portfolio.k_print,'[porfolio:init_portfolio] Failed to load benchmark file : ',benchmark_file)
                verbose(self._verbose,portfolio.k_print,'Evaluate its own benchmark')
                self.benchmark = portfolio('fixed_weight')
                self.benchmark._verbose = portfolio.k_error
                self.benchmark.init_portfolio(init_date = self.init_date\
                                        ,init_val = self.init_val\
    ...
    def get_status(self):
        ...
        if self.eval_bench and not self._benchmark_loaded:
            print (' |-benchmark: {g:3.2f}% (growth), {a:3.2f}% (annual)'\
    ...
    def elapse(self, interval:td = td(days=1)):
        ...
        while elapsed < interval:
            ...
            #if (not self.eval_bench) or (self.benchmark_file == None):
            if not self._benchmark_loaded:
                row[-3] = 0.0
            else:
                _benchmark_dates = self._benchmark_data['날짜'].values
                _date_it = self.date
                _benchmark_index = np.where(_benchmark_dates == _date_it)[0]
                while len(_benchmark_index) == 0:
                    if _date_it - td(days=1) < np.min(_benchmark_dates):
                        verbose(self._verbose, portfolio.k_warning, '[porfolio:elapse] There is no price information from benchmark data earlier than ',self.date)
                        _benchmark_val = self.init_val
                        break
                    else:
                        _date_it = _date_it - td(days=1)
                        _benchmark_index = np.where(_benchmark_dates == _date_it)[0]
                _benchmark_val = self._benchmark_data.iloc[_benchmark_index[0]]['종가']
                row[-3] = self.init_val*_benchmark_val/self._benchmark_init_val
            row[-2] = rf_yield
