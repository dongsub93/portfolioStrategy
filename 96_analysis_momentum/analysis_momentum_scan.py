...
tag_fmt = 'momentum_{t}ua{it}_rp{rp}_lp{lp}_rg{rg}'
    ...
    for gold in etf_gold:
        for bond in etf_bond:
            for stock_indx in etf_stock_indx:
                ...
                asset_weights = [0.]*len(underlying_assets)
                asset_weights[0] = 0.8
                asset_weights[1] = 0.2
...
#                        for rg in [1,5]:
                        for rg in [1]:
                            if rg >= lp or rp > lp:
                                continue
                            output_dir = './96_analysis_momentum/'+tag_fmt.format(t=tag, it=it,rp=rp,lp=lp,rg=rg)
                            ...
                            for init_shift in range(rp):
                                ...
                                command = 'pythonw analysis_momentum_backtest.py {od} {nt} {ut} {rp} {lp} {rg} {initd}'\
