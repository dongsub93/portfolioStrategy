...
tag_fmt = 'momentum_linear_{t}ua{it}_rp{rp}_lp{lp}_rg{rg}'
    ...
    for gold in etf_gold:
        for bond in etf_bond:
            for stock_indx in etf_stock_indx:
                ...
                n_assets = len(underlying_assets)
                asset_weights = [2.0*float((n_assets-i))/float(n_assets*(n_assets+1.0)) for i in range(n_assets) ]
...
#                        for rg in [1,5]:
                        for rg in [1]:
                            if rg >= lp or rp > lp:
                                continue
                            output_dir = './95_analysis_momentum_linear/'+tag_fmt.format(t=tag, it=it,rp=rp,lp=lp,rg=rg)
                            ...
                            for init_shift in range(rp):
                                ...
                                command = 'pythonw analysis_momentum_linear_backtest.py {od} {nt} {ut} {rp} {lp} {rg} {initd}'\
