...
class maximiseSR:
    ...
    def initialisePortfolio(self, initDate:datetime.date = datetime.date.today()\
        ...
        try:
            ...
            if self.initDate < max(_initDates):
                print ('[maximiseSR:initialisePortfolio]Initial point is even prior to the head of the data. Replace it with ',str(max(_initDates)))
            ...
            print ('[maximiseSR:loadHistory]Cannot find the price data. Set the path to the storage first.')
            return False
        ...
        for asset,price in _initPrices.items():
            if self.allowFrac:
                ...
                self.nAssets[asset] =float(\
                                           np.floor(\
                                                    self.initVal/(price*float(len(self.assetCodes)) )\
                                                   )\
                                          )
            _it += 1
    ...
    def loadHistory(self):
        ...
        for asset in self.assetCodes:
            try:
                ...
                raise ValueError('[maximiseSR:loadHistory] Cannot load price data of asset : {a}'.format(a=asset))
        ...
        try:
            ...
            self.riskFreeRatesData = None
            print ('[maximiseSR:loadHistory] Cannnot load the data of risk free rates.')
    ...
    def elapse(self,interval:datetime.timedelta=datetime.timedelta(days=1)):
        ...
        while _elapsed < interval:
            if any(self.date + datetime.timedelta(days=1) > _dates[0] for _dates in _assetDates.values() ):
                print ('[maximiseSR:elapse] Stop passing time. It reached to the end of the data.')
                break
            else:
        ...
        if self.evalBench:
            print ('[maximiseSR:elapse] Make benchmark portfolio.')
    ...
    def doRebalance(self):
        ...
        _scopeLen    = int(np.ceil(self.rebalScope/datetime.timedelta(days=1)))
        
        _returnArrays= {}
        for _asset, _returns in self.returns.items():
            _returnArrays[_asset] = np.array([])
            _idx = max(0,len(_returns)-_lookBackLen)
            while _idx <= len(_returns):
                if _idx + _scopeLen < len(_returns):
                    _returnArrays[_asset] = np.append(_returnArrays[_asset]\
                                                      ,np.product(_returns[_idx:_idx+_scopeLen]))
                    _idx = _idx + _scopeLen
                else:
                    _returnArrays[_asset] = np.append(_returnArrays[_asset]\
                                                      ,np.power( np.product(_returns[_idx:])\
                                                                ,_scopeLen/(_idx+_scopeLen-len(_returns)))\
                                                     )
                    break

                    
        _rfReturns = np.array([])
        _idx = max(0,len(self.rfReturns)-_lookBackLen)
        while _idx <= len(self.rfReturns):
            if _idx + _scopeLen < len(self.rfReturns):
                _rfReturns = np.append(_rfReturns\
                                        ,np.product(self.rfReturns[_idx:_idx+_scopeLen]))
                _idx = _idx + _scopeLen
            else:
                _rfReturns = np.append(_rfReturns\
                                       ,np.power( np.product(self.rfReturns[_idx:])\
                                                 ,_scopeLen/(_idx+_scopeLen-len(_returns)))\
                                      )
                break

        _assetWeights = dict([(_asset, 0.) for _asset in self.returns.keys() ])
        _assetWeights[self.assetCodes[0]] = 1.0
        _covMat = np.cov( np.array([ _returnArrays[self.assetCodes[_it]]\
                                        for _it in range(len(self.assetCodes))\
                                   ])\
                        )
                
        for _it in range(1,len(self.assetCodes)):
            _rf  = np.mean(_rfReturns)
            _r1  = 0.
            _cov = 0.
            _var1= 0.
            for j1 in range(_it):
                _r1  = _r1 + _assetWeights[self.assetCodes[j1]]*np.mean(_returnArrays[self.assetCodes[j1]])
                _cov = _cov+ _assetWeights[self.assetCodes[j1]]*_covMat[_it][j1]
                for j2 in range(_it):
                    _var1 = _var1 + _assetWeights[self.assetCodes[j1]]*_assetWeights[self.assetCodes[j2]]*_covMat[j1][j2]
                    
            _r2  = np.mean(_returnArrays[self.assetCodes[_it]])
            _var2= _covMat[_it][_it]
            _maxSR, _weight = maximiseSharpe(_rf,_r1,_r2,_var1,_var2,_cov)
            for j1 in range(_it):
                _assetWeights[self.assetCodes[j1]] = _assetWeights[self.assetCodes[j1]]*_weight
            _assetWeights[self.assetCodes[_it]] = 1- _weight
                
        if sum(_assetWeights.values()) == 0.0:
            print ('[maximiseSR:doRebalance] {d}, All assets have returns less than risk-free asset({r:3.2f}) in the loop-back period.'\
                   .format(d=self.date,r=100*_rfAverage))
        
        for _asset in self.assetCodes:
            if self.allowFrac:
                self.nAssets[_asset] = self.portfolioValue*_assetWeights[_asset]/_assetPricesToday[_asset]
            else:
                self.nAssets[_asset] = np.floor(self.portfolioValue*_assetWeights[_asset]/_assetPricesToday[_asset])
            self.tAssets[_asset] = self.nAssets[_asset]*_assetPricesToday[_asset]
        self.cash = self.portfolioValue - sum(self.tAssets.values())
    ...
    def getHistory(self, start=None, end:datetime.date=datetime.date.today()):
        ...
        if start > end:
            print ('[maximiseSR:getHistory]The argument indicating the last date of the history is prior than start date. Switching them.')
            buffer = start
            start = end
            end = buffer
            del buffer
        ...
        while len(_indxStart) == 0:
            if start - datetime.timedelta(days=1) < _historyDates[0]:
                print ('[maximiseSR:getHistory] There is no day included in the interval.')
                return None
            else:
