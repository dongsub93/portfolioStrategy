...
class fixedWeightsStrategy:
    ...
    def doRebalance(self):
        self.datesFromLastRebal = datetime.timedelta(days=0)
        _rebalScopeLen = int(np.ceil(self.lookBackPeriod/datetime.timedelta(days=1)))
        ...
        for asset, assetData in self.assetData.items():
            ...
            _meanPrices[asset] = np.product(assetData.loc[max(_todayIndx-_rebalScopeLen,0):_todayIndx+1,'종가'].values)
        ...
        for asset in _meanPrices.keys():
            if self.allowFrac:
                ...
                self.nAssets[asset] = np.floor(self.portfolioValue*self.assetWeights[_rank]/_assetPricesToday[asset])
            _rank += 1
