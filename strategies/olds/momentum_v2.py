...
import pandas as pd
import numpy as np
import aux_functions_v1
from aux_functions_v1 import getMDD
...
class momentumStrategy:
    def __init__(self , initDate:datetime.date = datetime.date.today()\
        ...
        if len(self.assetCodes) != 0:
            self.benchmark.initialisePortfolio(initDate = self.initDate, initVal      = self.initVal\
                                 ...
                                 ,rebalPeriod  = self.rebalPeriod, lookBackPeriod= self.lookBackPeriod\
                                 ,allowFrac    = False, storageDir    = self.storageDir)
        ...
        _dfColumns = np.append(_dfColumns,['cash','benchmarkValue','rebalancing'])
    ...
    def initialisePortfolio(self, initDate:datetime.date = datetime.date.today()\
        ...
        self.portfolioValue= self.initVal
        self.benchmark     = fixedWeightsStrategy()
        self.benchmark.initialisePortfolio(initDate = self.initDate\
        ...
        _row = [None]*(5+3*len(self.assetCodes))
    ...
    def verbStatus(self):
        ...
        _bMDD, bMDDfrom, _bMDDto = getMDD(self.history, 'benchmarkValue')
    ...
    def doRebalance(self):
        ...
        if sum(_assetWeights.values()) == 0.0:
            print ('[momentumStrategy:doRebalance] {d}, All assets show returns less than 1 in the scope.'.format(d=self.date))
        else:
